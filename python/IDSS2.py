...
class IDSS():
    ...
    def findAllValidTriples(self):
        ...
        for permu in permutations:
            ...
            if error == 0:
                ...
                net = SeriationGraph()
                propHash={"Name":str(numberOfTriplets),"End1":str(permu[0]),"End2":str(permu[2]),"Middle":str(permu[1])}
                net.set_graph_properties(propHash)
                propHash={}
                graphID = net.get_graph_ID()

                n1 = net.add_node(permu[0], permu[0])
                propHash={ "site":"end", "end":1, "connectedTo":permu[1]}
                net.set_node_properties(n1,propHash)

                n2= net.add_node(permu[1], permu[1])
                propHash={ "site":"middle", "end":0, "connectedTo":"middle"}
                net.set_node_properties(n2,propHash)

                n3= net.add_node(permu[2], permu[2])
                propHash={ "site":"end", "end":1, "connectedTo":permu[1]}
                net.set_node_properties(n3,propHash)

                e1=net.add_edge(n1,n2)
                #permu[0], permu[1])
                edgeHash={"weight":comparison12, "GraphID":graphID, "end":1}
                net.set_edge_properties(e1,edgeHash)
                e2=net.add_edge(n3,n2)
                #permu[2], permu[1])
                edgeHash={"weight":comparison23, "GraphID":graphID, "end":1}
                net.set_edge_properties(e2,edgeHash)
                ...
                vlist,elist = net.find_path(net.nodeHash(permu[0]),net.nodeHash(permu[2]))

                logger.debug("VALID TRIPLE SOLUTION: Shortest Path: %s ", vlist)
                triples.append(net)
                numberOfTriplets += 1
    ...
    def checkForValidAdditionsToNetwork(self, nnetwork):
        ...
        logger.debug(
            ...
            nnetwork.graph_properties['GraphID'], len(nnetwork.nodeHash))
        if self.args['screen'] not in self.FalseList:
            self.scr.addstr(1, 40, "STEP: Testing for addition to seriation ....      ")
            self.scr.refresh()

        logger.debug("The end of assemblages of network %d are: %s and %s", nnetwork.graph_properties['GraphID'],
                     nnetwork.get_graph_property("End1"), nnetwork.graph_properties["End2"])
        logger.debug("Network:  %s", nnetwork.adjacency_list())
        vlist,elist = gt.shortest_path(nnetwork,nnetwork.graph_properties("End1"),nnetwork.graph_properties("End2"))
        logger.debug("Seriation %d to evaluate: Shortest Path: %s ", nnetwork.graph_properties['GraphID'],vlist)
        array_of_new_networks = []  ## a list of all the valid new networks that we run into
        maxnodes = len(nnetwork.vertices())
        ...
        for assEnd in ("End1", "End2"):
            ...
            endAssemblage = nnetwork.graph_properties[assEnd]
            logger.debug(">>>>>> Checking ends of seriation %d:  %s is %s", nnetwork.graph_properties['GraphID'], assEnd,
                         endAssemblage)
            ...
            for testAssemblage in validAssemblages:
                ...
                logger.debug("End assemblage for this seriation: %s", nnetwork.graph_properties[assEnd])
                logger.debug("Which end: %s", assEnd)
                innerNeighbor = None
                if assEnd == "End1":
                    vlist,elist = gt.shortest_path(nnetwork,nnetwork.graph_properties("End1"),nnetwork.graph_properties("End2"))
                    #path = nx.shortest_path(nnetwork, nnetwork.graph["End1"], nnetwork.graph["End2"])
                    innerNeighbor = vlist[1]
                    logger.debug("End1: %s Neighbor: %s", endAssemblage, innerNeighbor)
                elif assEnd == "End2":
                    vlist,elist = gt.shortest_path(nnetwork,nnetwork.graph_properties("End2"),nnetwork.graph_properties("End1"))
                    ...
                    vlist,elist = gt.shortest_path(nnetwork,nnetwork.graph_properties("End1"),nnetwork.graph_properties("End2"))
                    print "For this network:  ", vlist

                    #    nx.shortest_path(nnetwork, nnetwork.graph["End1"],
                    #                                              nnetwork.graph["End2"])
                ...
                logger.debug("\t\t\tThere should be just 1 neighbor to %s and that is: %s", endAssemblage,
                             innerNeighbor)
                e = self.pairGraph.edge(innerNeighbor,endAssemblage)
                #c = e.get_edge_data(innerNeighbor, endAssemblage)
                comparison = e.edge_properties['weight']

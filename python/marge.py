...
## open the data file
useOutputFile = substr(inputfile,0,-4);
...
## We use the Math::Combinatorics to get all the combinations of 2
pairs = Math::Combinatorics->new( count => 2, data => [@assemblageNumber] );
## Go through all of the combinations
while ( my @permu = pairs->next_combination ) {
    #DEBUG and print labels[ permu[0] ] . " * " . labels[ permu[1] ] . "\n";
    pairname      = labels[ permu[0] ] . " * " . labels[ permu[1] ];
    pairname2     = labels[ permu[1] ] . " * " . labels[ permu[0] ];
    maxDifference = 0;
    for ( i = 0 ; i < cols ; i++ ) {
...
if ( filterflag == 1 ) {
   ...
   for (i=scalar(@solutions)-1;i>-1;i--) {
      exists=0;

...
maxnumber = 0
count = 0
screen = 1    ## if the noscreen option is 1, then set the screen flag to 0. Otherwise use the screen
if noscreen is True:
    screen=0
if DEBUG is True:
    screen=0
...
## open the data file
useOutputFile = inputfile[0,-4]
file= open( inputfile, 'rb' )
OUTFILE = open(">useOutputFile.vna" )
...
if screen:
    scr.addstr(1,1,"Filename:  inputfile")
    scr.addstr(2,1,"Threshold: threshold")
...
if screen:
    scr->at(3,1)
    scr->puts("Maximum possible seriation solution length: count")
...
if (xyfile) {
   ...
   assemblagePairs = Math::Combinatorics->new( count => 2, data => [@xyAssemblages] )
   ...
   while ( my @combo = assemblagePairs->next_combination ) {
      ...
      distance = sqrt( (xAssemblage{ combo[0] } -xAssemblage{combo[1]})**2 + (yAssemblage{combo[0]} -yAssemblage{combo[1]})**2) 
...
if (bootstrapCI) {
    ...
    foreach currentLabel ( sort keys %assemblageFrequencies) {
        ...
        my @arrayOfStats = ()
        stat   ## this will be the stat objects
        ...
        while (loop) {
            ...
            classCount=0
            foreach stat (sort @arrayOfStats) {
...
foreach label ( @labels ) {
    my @cAssemblages
    foreach comparativeLabel (@labels ) {
...
permutations =Math::Combinatorics->new( count => 3, data => [@assemblageNumber] )
...
## list of valid triples is @triples.
...
maxEdges  = 2      ## keeps track of the current largest size of solution
stepcount = 0     ## keeps track of the current step (from 0, adds one each step)
match     = 0      ## keeps track of whether a solution is found for each step. Once no new solution is found, the search is over!
...
solutionCount=scalar(@triples)   ## an index of the current number of solutions
...
while ( currentMaxSeriationSize <= maxSeriations ) {
    ...
    foreach nnetwork (@networks) {
        ...
        foreach endAssemblage (nnetwork->get_graph_attribute("End_1"), nnetwork->get_graph_attribute("End_2")) {
    ...
    if ( scalar(@newnets) == 0 ) {
        text = "Max seriation size reached - Largest solution set: ". scalar(@networks). " out of ". solutionCount
        screen and scr->at(9,1)->puts( text )
...
if ( filterflag == 1 ) {
   ...
   for (i=scalar(@solutions)-1i>-1i--) {
      exists=0
      foreach tnetwork (@filteredarray) {
    ...
    filterCount= scalar(@filteredarray)
...
if (individualfileoutput) {
    ...
    count = 0
    name
    foreach network (@filteredarray) {
        E = network->unique_edges
        if ( E > maxnumber - 1 ) {
            count++
            name = count . '.vcg'
...
foreach l (@labels) {
    #print OUTFILE l, "\n"
    x = xAssemblage{ l }/1000000 || 0
    y = (largestY-yAssemblage{ l })/100000 || 0
    easting = xAssemblage{ l } || 0
    northing = yAssemblage{ l } || 0
...
if (mst) {
    megaNetwork = Graph::Undirected->new
    distanceNetwork = Graph::Undirected->new
    ...
    foreach network (@uniqueArray) {
        ...
        eCount
        my @Edges = network->unique_edges
        graphID = network->get_graph_attribute("GraphID")
        groupDistance=0
        meanDistance=0.0
        ...
        foreach e (@Edges) {
            ...
            distpairname= edge0."*".edge1
            distanceNetwork->add_vertex( edge0 )
            distanceNetwork->add_vertex( edge1 )
            distance = distanceBetweenAssemblages{ distpairname }
    ...
    mstgBootstrap = megaNetwork->minimum_spanning_tree
    count=1
    ...
    mstgDistance = distanceNetwork->minimum_spanning_tree
    count=1
...
if (xyfile ) {
    ...
    graph = new GD::Graph::histogram(400,600)
    graph->set(
...
if (stats>0 ) {
    ...
    my @data
    foreach key (keys %vertHash){
        push @{data[0]},key
        push @{data[1]}, vertHash{ key }
    }
    graph = new GD::Graph::bars(400,600)
    graph->set(
                x_label         => 'Assemblages',
                y_label         => 'Count',
...
if (excel) {
    ...
    filename = useOutputFile."-seriationList.xlsx"
    workbook = Excel::Writer::XLSX->new( filename )

    # Add a worksheet
    worksheet = workbook->add_worksheet()
    percentWorksheet = workbook->add_worksheet()
    row = 1
    solutionNumber=1
    foreach network (@uniqueArray) {
        if (ref(network) eq 'REF') {
        ...
        #undirectedNetwork = network->undirected_copy
        col=0
        if (largestonly>0) {
            if (network->unique_edges == maxEdges) {
...
timediff= Time::HiRes::gettimeofday() - start
...
sub SortHashByMultipleColumns{
    ...
    sortfunc = eval "sub { sortCriteria }"
...
def usage():
    ...
    This program reads given input file(s) and create seriations.
